...
else
{
    ...
    dotnet restore Pecuniary.Transaction.Command/Pecuniary.Transaction.Command.csproj
    
    Write-Host "`n`nBuild:"
    
	dotnet publish Pecuniary.Transaction.Command/Pecuniary.Transaction.Command.csproj
}
  
Write-Host "`n`nDeploy:"

dotnet-lambda deploy-serverless `
    --stack-name $developerPrefix-pecuniary-transactioncommand-stack `
...
$stack = aws cloudformation describe-stacks --stack-name $developerPrefix-pecuniary-transactioncommand-stack | ConvertFrom-Json
...
Write-Host "`n`n YOUR STACK NAME IS:   $developerPrefix-pecuniary-transactioncommand-stack   `n`n"
